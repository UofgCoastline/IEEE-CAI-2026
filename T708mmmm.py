...
class EnhancedCoastlineAgent:
    ...
    def apply_enhanced_inference(self, max_inference_steps=1800):
        """应用增强版推理算法 - 先产生丰富结果再智能过滤"""
        print("🔮 使用边缘检测引导DQN进行海岸线分割（先丰富后智能过滤）...")
        ...
        # 保存丰富的初始结果
        rich_initial_result = self.env.current_coastline.copy()
        initial_pixels = np.sum(rich_initial_result > 0.3)
        print(f"   📊 丰富初始结果: {initial_pixels:,} 像素")

        # 智能比较逻辑：与Enhanced Land Detection对比选择最优结果
        final_result = self._smart_comparison_selection(rich_initial_result)
    ...
    def _smart_comparison_selection(self, rich_initial_result):
        """智能比较选择 - 与Enhanced Land Detection对比选择最优结果"""
        print("   🧠 ========== 智能比较选择 ==========")

        # 获取Enhanced Land Detection的像素数量作为参考基准
        land_mask = self.env.enhanced_analysis['land_mask']
        land_pixels = np.sum(land_mask)
        print(f"   📊 Enhanced Land Detection基准: {land_pixels:,} 像素")

        # 候选1：丰富的初始结果
        candidate1 = rich_initial_result.copy()
        candidate1_pixels = np.sum(candidate1 > 0.3)

        # 候选2：进行轻微过滤的结果
        candidate2 = self._light_filtering(rich_initial_result)
        candidate2_pixels = np.sum(candidate2 > 0.3)

        print(f"   📊 候选结果对比:")
        print(f"       候选1 (丰富结果): {candidate1_pixels:,} 像素")
        print(f"       候选2 (轻微过滤): {candidate2_pixels:,} 像素")
        print(f"       参考基准 (Land Detection): {land_pixels:,} 像素")
        ...
        print(f"   📊 与基准差距:")
        print(f"       候选1差距: {diff1:,} 像素")
        print(f"       候选2差距: {diff2:,} 像素")

        # 选择更接近基准的结果
        if diff1 <= diff2:
            selected_result = candidate1
            selected_pixels = candidate1_pixels
            selection_reason = f"丰富结果更接近基准 (差距: {diff1:,})"
        else:
            selected_result = candidate2
            selected_pixels = candidate2_pixels
            selection_reason = f"轻微过滤更接近基准 (差距: {diff2:,})"

        print(f"   ✅ 选择结果: {selection_reason}")
        print(f"   📊 最终像素: {selected_pixels:,}")
        print("   🧠 ========== 智能比较完成 ==========")
    ...
    def _light_filtering(self, initial_result):
        ...
        for y, x in zip(coastline_positions[0], coastline_positions[1]):
            ...
            # 非常严格的噪声定义：边缘和边界置信度都很低
            if edge_val < 0.02 and boundary_val < 0.02:
                noise_pixels.append((y, x))

        # 轻微过滤：只移除最明显的噪声，最多移除5%
        max_remove = min(len(noise_pixels), len(coastline_positions[0]) // 20)

        filtered_result = initial_result.copy()

        for i in range(max_remove):
            y, x = noise_pixels[i]
            filtered_result[y, x] *= 0.1  # 大幅降低而不是完全删除

        removed_pixels = np.sum(initial_result > 0.3) - np.sum(filtered_result > 0.3)
        print(f"       轻微过滤移除: {removed_pixels} 个明显噪声像素")
...
def create_enhanced_uk_visualization(result, save_path):
    ...
    # 创建更丰富的海域误识别检测（基于原始丰富结果）
    # 使用更低的阈值来显示更多候选像素
    rich_coastline_binary = (result['coastline_result'] > 0.15).astype(float)  # 降低阈值

    # 基于丰富结果检测海域误识别
    ocean_cleaner = OceanMisclassificationCleaner()

    # 如果原始结果像素太少，使用边缘检测结果
    if np.sum(rich_coastline_binary) < 5000:
        # 直接使用边缘检测结果作为候选
        advanced_edges = result['enhanced_analysis']['advanced_edges']
        edge_candidates = (advanced_edges > 0.03).astype(float)
        ocean_false_coastlines = edge_candidates * color_ocean_mask.astype(float)
    else:
        ocean_false_coastlines = ocean_cleaner._detect_ocean_false_coastlines(
            rich_coastline_binary, color_ocean_mask, result['processed_image']
        )
