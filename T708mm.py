...
class EnhancedCoastlineAgent:
    ...
    def apply_enhanced_inference(self, max_inference_steps=1800):
        """应用增强版推理算法 - 智能渐进过滤版"""
        print("🔮 使用边缘检测引导DQN进行海岸线分割（智能渐进过滤）...")
        ...
        # 生成初始检测结果（类似左边的图）
        initial_result = self._generate_initial_detection(inference_positions, advanced_edges)

        # 智能渐进过滤判断
        final_result = self._intelligent_progressive_filtering(initial_result)

        self.env.current_coastline = final_result
        final_pixels = np.sum(final_result > 0.3)

        print(f"   ✅ 智能渐进过滤完成: {final_pixels:,} 像素")
