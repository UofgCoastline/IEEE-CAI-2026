...
def main():
    ...
    # 运行测试
    result = test_precise_sea_cleanup()

    if result:
        print(f"\n✅ 精准海域清理处理完成!")
        print(f"   最终像素数: {result['quality_metrics']['coastline_pixels']:,}")
        print(f"   是否在目标范围(9-10万): {'✅' if 90000 <= result['quality_metrics']['coastline_pixels'] <= 100000 else '❌'}")
        ...
        if result['final_coastline'] is not None:
            ...
            total_pixels = result['quality_metrics'][('coastline_pixels'
                                                      ''
                                                      '')]
