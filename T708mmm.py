...
class EnhancedCoastlineAgent:
    ...
    def apply_enhanced_inference(self, max_inference_steps=1800):
        ...
        # 第二阶段：智能渐进过滤判断
        final_result = self._intelligent_progressive_filtering(rich_initial_result)
    ...
    def _intelligent_progressive_filtering(self, initial_result):
        ...
        while filter_iteration < max_iterations:
            ...
            print(f"   📊 第 {filter_iteration + 1} 轮一致性评分: {consistency_score:.1%}")
            ...
            # 如果一致性不足，进行小幅度过滤
            print(f"   🔧 一致性不足 ({consistency_score:.1%} < 70%)，进行小幅度过滤...")
            current_result = self._gentle_filtering(current_result, advanced_edges,
                                                    boundary_confidence, color_consistency)

            filter_iteration += 1

        if filter_iteration >= max_iterations:
            print(f"   ⚠️ 达到最大过滤轮数 ({max_iterations})，停止过滤")

        final_pixels = np.sum(current_result > 0.3)
        print(f"   📊 最终像素数量: {final_pixels:,}")
    ...
    def _calculate_consistency_score(self, result, edges, boundary, color_consistency):
        ...
        # 计算综合一致性得分（边缘检测权重更高）
        edge_score = min(1.0, avg_edge / 0.3)  # 边缘检测期望值
        boundary_score = min(1.0, avg_boundary / 0.2)  # 边界置信度期望值
        color_score = min(1.0, (1.0 - avg_color_consistency) / 0.5)  # 色彩不一致期望值

        # 加权平均（边缘检测权重最高）
        consistency_score = (
                edge_score * 0.5 +  # 边缘检测 50%
                boundary_score * 0.3 +  # 边界置信度 30%
                color_score * 0.2  # 色彩一致性 20%
        )

        return min(1.0, consistency_score)

    def _gentle_filtering(self, result, edges, boundary, color_consistency):
        """温和的过滤 - 只移除最不一致的像素"""
        binary_result = (result > 0.3).astype(bool)
        coastline_positions = np.where(binary_result)
        ...
        for i, (y, x) in enumerate(zip(coastline_positions[0], coastline_positions[1])):
            ...
            # 不一致程度：边缘弱、边界置信度低、色彩一致性高（不像边界）
            inconsistency = (
                    (0.3 - edge_val) * 0.5 +  # 边缘越弱越不一致
                    (0.2 - boundary_val) * 0.3 +  # 边界置信度越低越不一致
                    (color_val - 0.5) * 0.2  # 色彩越一致越不像边界
            )

            inconsistency_scores.append((inconsistency, y, x))

        # 按不一致程度排序
        inconsistency_scores.sort(reverse=True, key=lambda x: x[0])

        # 只移除最不一致的10%像素（温和过滤）
        remove_count = max(1, len(inconsistency_scores) // 10)

        filtered_result = result.copy()
        removed_count = 0

        for inconsistency, y, x in inconsistency_scores[:remove_count]:
            if inconsistency > 0:  # 只移除确实不一致的像素
                filtered_result[y, x] *= 0.3  # 降低置信度而不是完全删除
                removed_count += 1

        print(f"      🔧 温和过滤移除了 {removed_count} 个不一致像素")
