...
class EnhancedCoastlineEnvironment:
    ...
    def apply_enhanced_post_processing(self):
        ...
        # 3. 最终优化
        self.current_coastline = self._final_optimization(enhanced_coastline)

        return self.current_coastline

    def _final_optimization(self, coastline_result):
        """最终优化"""
        # 中值滤波去噪
        optimized = median_filter(coastline_result, size=3)

        # 保留强边缘
        strong_edges = coastline_result > 0.8
...
class EnhancedCoastlineAgent:
    ...
    def apply_enhanced_inference(self, max_inference_steps=1800):
        ...
        print(f"   🎯 智能搜索位置数: {len(candidate_positions)}")

        # 增强版智能位置优先级排序
        prioritized_positions = self._enhanced_priority_sorting(candidate_positions)
        ...
        # 限制推理位置数
        inference_positions = prioritized_positions[:max_inference_steps]
        print(f"   🎯 最终推理位置数: {len(inference_positions)}")

        # 分阶段推理
        stages = [
            {'name': '高优先级', 'positions': inference_positions[::3], 'threshold': 20.0},
            {'name': '中优先级', 'positions': inference_positions[1::3], 'threshold': 15.0},
            {'name': '低优先级', 'positions': inference_positions[2::3], 'threshold': 10.0}
        ]

        for stage_idx, stage in enumerate(stages):
            print(f"   🔄 第 {stage_idx + 1} 阶段推理: {stage['name']}")
            ...
            for position in stage['positions']:
                ...
                # 动态阈值判断
                if reward > stage['threshold']:
                    # 根据奖励调整更新值
                    update_value = min(1.0, reward / 60.0)
                    self.env.update_coastline(next_position, update_value)
                    total_improvements += 1
                    stage_improvements += 1

            print(f"      ✅ 第 {stage_idx + 1} 阶段改进: {stage_improvements}")

        # 应用增强版后处理
        print("   🔧 应用增强版后处理...")
        enhanced_coastline = self.env.apply_enhanced_post_processing()

        final_pixels = np.sum(enhanced_coastline > 0.3)
        avg_reward = total_reward / len(inference_positions) if inference_positions else 0

        print(f"   ✅ 增强版推理完成: {final_pixels:,} 像素, 总改进: {total_improvements}")
