...
class GroundTruthComparator:
    ...
    def __init__(self, gt_directory="E:/Other_test"):
    ...
    def process_ground_truth(self, gt_image):
        """处理Ground Truth图像，提取海域和陆域 - 完全无OpenCV版本"""
        print("   🔄 处理Ground Truth图像...")

        # 🔧 修复：使用手动RGB到灰度转换，完全替代cv2
        gt_gray = self.rgb_to_gray_manual(gt_image)

        # 🔧 修复：使用手动二值化，完全替代cv2.threshold
        gt_binary = self.threshold_manual(gt_gray, 128)

        # 白色(255)代表陆域，黑色(0)代表海域
        land_mask = (gt_binary == 255)
        water_mask = (gt_binary == 0)

        print(f"   📊 GT统计:")
        print(f"       陆域像素: {np.sum(land_mask):,}")
        print(f"       海域像素: {np.sum(water_mask):,}")
        print(f"       陆域比例: {np.sum(land_mask) / gt_binary.size:.1%}")
    ...
    def extract_edge_regions(self, gt_processed, edge_width=10):
        """提取海岸边缘区域"""
        print(f"   🔍 提取海岸边缘区域 (宽度: {edge_width}像素)...")

        land_mask = gt_processed['land_mask']
        water_mask = gt_processed['water_mask']

        # 方法1：基于陆域边界
        land_eroded = binary_erosion(land_mask, np.ones((edge_width, edge_width)))
        land_edge = land_mask & ~land_eroded

        # 方法2：基于海域边界
        water_eroded = binary_erosion(water_mask, np.ones((edge_width, edge_width)))
        water_edge = water_mask & ~water_eroded

        # 结合两种方法
        edge_region = land_edge | water_edge

        # 进一步扩展边缘区域
        edge_region = binary_dilation(edge_region, np.ones((3, 3)))

        edge_pixels = np.sum(edge_region)
        edge_ratio = edge_pixels / edge_region.size

        print(f"   📊 边缘区域统计:")
        print(f"       边缘像素: {edge_pixels:,}")
        print(f"       边缘比例: {edge_ratio:.1%}")
    ...
    def evaluate_subregion(self, subregion, predicted_coastline, gt_processed, edge_region):
        ...
        gt_land_region = gt_processed['land_mask'][y_start:y_end, x_start:x_end]
        ...
        # 预测的海岸线在边缘区域内
        pred_edge = pred_region > 0.3  # 预测的海岸线
        gt_edge = ~gt_land_region  # GT中的海域（非陆域）

        # 只在边缘区域内比较
        pred_edge_masked = pred_edge & edge_region_sub
        gt_edge_masked = gt_edge & edge_region_sub
        ...
        # 计算准确率指标
        pred_flat = pred_edge_masked.flatten()
        gt_flat = gt_edge_masked.flatten()
        edge_flat = edge_region_sub.flatten()

        # 只计算边缘区域内的准确率
        pred_edge_only = pred_flat[edge_flat]
        gt_edge_only = gt_flat[edge_flat]

        if len(pred_edge_only) == 0:
            return None

        try:
            accuracy = accuracy_score(gt_edge_only, pred_edge_only)
            precision = precision_score(gt_edge_only, pred_edge_only, zero_division=0)
            recall = recall_score(gt_edge_only, pred_edge_only, zero_division=0)
            f1 = f1_score(gt_edge_only, pred_edge_only, zero_division=0)
        except Exception as e:
            print(f"   ⚠️ 计算指标失败: {e}")
            return None
        ...
        return {
            ...
            'recall': recall,
            'f1': f1,
            'pred_positive': np.sum(pred_edge_only),
            'gt_positive': np.sum(gt_edge_only)
        }
...
if __name__ == "__main__":
    ...
    # 方式3: 仅快速测试增强版 v2.0
    quick_test_enhanced_v2_with_gt()
