...
class ColorSensitivityFilter:
    ...
    def create_color_based_mask(self, rgb_image):
        ...
        lab_features = self._rgb_to_lab_features(rgb_image)

        # 1. 蓝色系海域检测（更精确）
        blue_sea_mask = self._detect_blue_sea_regions(rgb_image, hsv_image)

        # 2. 深色海域检测
        dark_sea_mask = self._detect_dark_sea_regions(rgb_image, hsv_image)

        # 3. 色彩聚类分析
        clustered_sea_mask = self._color_clustering_sea_detection(rgb_image)

        # 4. 纹理一致性分析
        texture_sea_mask = self._texture_consistency_analysis(rgb_image)

        # 综合海域掩膜
        combined_sea_mask = blue_sea_mask | dark_sea_mask | clustered_sea_mask | texture_sea_mask

        # 形态学优化
        combined_sea_mask = self._morphological_sea_optimization(combined_sea_mask)
...
class OceanMisclassificationCleaner:
    ...
    def clean_ocean_misclassifications(self, coastline_result, rgb_image, hsv_analysis):
        ...
        # 1. 创建高精度海域掩膜
        precise_ocean_mask = self._create_precise_ocean_mask(rgb_image, hsv_analysis)

        # 2. 检测海域内的误识别海岸线
        ocean_false_coastlines = self._detect_ocean_false_coastlines(
            coastline_result, precise_ocean_mask, rgb_image
        )

        # 3. 基于色彩相似性的清理
        color_based_cleaning = self._color_similarity_cleaning(
            coastline_result, rgb_image, ocean_false_coastlines
        )

        # 4. 距离基础的清理
        distance_based_cleaning = self._distance_based_cleaning(
            color_based_cleaning, precise_ocean_mask
        )

        # 5. 最终整合清理
        cleaned_coastline = self._final_integration_cleaning(
            distance_based_cleaning, coastline_result, precise_ocean_mask
        )

        cleaning_ratio = np.sum(ocean_false_coastlines) / (np.sum(coastline_result > 0.3) + 1e-8)
        print(f"   ✅ 清理完成，移除了 {cleaning_ratio:.1%} 的误识别像素")
...
class EnhancedCoastlineEnvironment:
    ...
    def apply_enhanced_post_processing(self):
        """应用轻量级后处理 - 保留更多边缘引导结果"""
        print("   🔧 应用轻量级后处理（保留边缘引导结果）...")

        # 跳过海域误识别清理，因为我们相信边缘检测的准确性
        # cleaned_coastline = self.ocean_cleaner.clean_ocean_misclassifications(
        #     self.current_coastline, self.image, self.enhanced_analysis
        # )

        # 跳过边缘精准度增强，因为已经基于边缘检测
        # enhanced_coastline = self.edge_enhancer.enhance_edge_precision(
        #     cleaned_coastline, self.image, self.enhanced_analysis
        # )

        # 只进行最基础的优化
        self.current_coastline = self._minimal_optimization(self.current_coastline)

        return self.current_coastline

    def _minimal_optimization(self, coastline_result):
        """最小化优化 - 只保留必要的处理"""
        # 轻微的中值滤波去噪（减小核心大小）
        optimized = median_filter(coastline_result, size=2)

        # 保留所有强边缘（降低阈值）
        strong_edges = coastline_result > 0.5
        optimized[strong_edges] = coastline_result[strong_edges]

        # 轻微的连续性增强
        advanced_edges = self.enhanced_analysis['advanced_edges']
        edge_positions = np.where(advanced_edges > 0.15)

        for y, x in zip(edge_positions[0], edge_positions[1]):
            # 如果周围有海岸线像素，且当前位置边缘强度高，则增强
            if self._has_nearby_coastline(optimized, y, x, radius=2):
                edge_strength = advanced_edges[y, x]
                optimized[y, x] = max(optimized[y, x], edge_strength * 0.6)
    ...
    def _has_nearby_coastline(self, coastline, y, x, radius=2):
        """检查周围是否有海岸线像素"""
        y_start, y_end = max(0, y - radius), min(coastline.shape[0], y + radius + 1)
        x_start, x_end = max(0, x - radius), min(coastline.shape[1], x + radius + 1)

        local_region = coastline[y_start:y_end, x_start:x_end]
        return np.any(local_region > 0.3)
    ...
    def _calculate_enhanced_reward(self, old_pos, new_pos, action_idx):
        ...
        reward += boundary_confidence * 60.0

        # 海岸线指导奖励
        guidance_score = self.enhanced_analysis['coastline_guidance'][y, x]
        reward += guidance_score * 50.0

        # 增强版NDWI奖励
        enhanced_ndwi = self.enhanced_analysis['enhanced_ndwi'][y, x]
        ndwi_reward = max(0, 25.0 - abs(enhanced_ndwi) * 35.0)
        reward += ndwi_reward

        # 色彩一致性奖励（新增）
        color_consistency = self.enhanced_analysis['color_consistency'][y, x]
        if color_consistency < 0.5:  # 一致性低的地方可能是边界
            reward += (1.0 - color_consistency) * 20.0
        ...
        # 增强版水陆分离奖励
        separation_reward = self._calculate_enhanced_separation_reward(new_pos)
        reward += separation_reward

        # 边缘质量奖励（增强版）
        edge_strength = self.edge_map[y, x]
        reward += edge_strength * 30.0

        # 避免深海区域（更严格）
        water_mask = self.enhanced_analysis['water_mask']
        if water_mask[y, x] and enhanced_ndwi > 0.4:
            reward -= 40.0

        # 色彩敏感度惩罚（新增）
        color_penalty = self._calculate_color_sensitivity_penalty(new_pos)
...
class EnhancedCoastlineAgent:
    ...
    def apply_enhanced_inference(self, max_inference_steps=1800):
        ...
        # 基于边缘强度创建候选位置（阈值调低，保留更多边缘）
        edge_threshold = 0.1  # 降低阈值，保留更多边缘
        ...
        # 按边缘强度排序（强边缘优先）
        edge_guided_positions = self._edge_strength_sorting(candidate_positions, advanced_edges)
        ...
        # 限制处理位置数
        inference_positions = edge_guided_positions[:max_inference_steps]
        ...
        for i, position in enumerate(inference_positions):
            ...
            # 边缘引导的更新策略：直接基于边缘强度更新
            # 边缘强度越高，置信度越高
            update_value = edge_strength * 0.8 + min(1.0, max(0, reward / 30.0)) * 0.2

            # 更宽松的更新条件 - 只要有一定边缘强度就更新
            if edge_strength > 0.15 or reward > 5.0:
        ...
        # 边缘连续性增强（保持更多连续边缘）
        print("   🔧 应用边缘连续性增强...")
        self._enhance_edge_continuity_guided()
