...
class HSVAttentionSupervisor:
    ...
    def _define_land_hsv_range(self):
        ...
        return {
            ...
            'saturation_min': 0.2,  # 最小饱和度
            'value_min': 0.3  # 最小明度
        }

    def analyze_image_hsv(self, rgb_image):
        """分析图像的HSV特征"""
        if len(rgb_image.shape) == 3:
            # 转换为HSV
    ...
    def _generate_coastline_guidance(self, water_mask, land_mask):
        ...
        guidance_strength = np.exp(-0.1 * (water_dist + land_dist))
    ...
    def _calculate_transition_strength(self, hsv_image, water_mask, land_mask):
        ...
        transition_strength = transition_strength * (1 + boundary_mask * 2)
...
class ConstrainedCoastlineEnvironment:
    ...
    def __init__(self, image, gt_analysis):
        ...
        self.hsv_analysis = self.hsv_supervisor.analyze_image_hsv(image)
    ...
    def _setup_constrained_search_region(self):
        ...
        # 扩展搜索区域，但限制在合理范围内
        self.search_region = primary_region.copy()
        for _ in range(2):  # 减少扩展次数
        ...
        if self.gt_analysis and self.gt_analysis['gt_binary'] is not None:
            if self.gt_analysis['gt_binary'][y, x]:
                ...
                if len(gt_coords[0]) > 0:
                    ...
                    if min_dist <= 3:
                        reward += 30.0 - min_dist * 5.0
                    elif min_dist <= 8:
                        reward += 20.0 - min_dist * 2.0
