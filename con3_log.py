...
class UltraEnhancedCoastlineDetector:
    ...
    def advanced_spatial_suppression(self, coastlines, color_regions):
        ...
        basic_suppression_zones = self._create_suppression_zones(ocean_depth_map, blue_mask, land_mask)

        # 第二步：海域内部完全抑制策略
        ocean_interior_suppression = self._create_ocean_interior_suppression(coastlines, color_regions)

        # 合并抑制区域
        combined_suppression = np.maximum(basic_suppression_zones, ocean_interior_suppression)

        # 对每个海岸线检测结果应用抑制
        suppressed_coastlines = {}
        for name, coastline in coastlines.items():
            if name in ['learned_from_gt', 'enhanced_edges']:
                suppression_factor = 0.2  # 保护重要特征，但仍然抑制海域内部
            else:
                suppression_factor = 0.9  # 强抑制

            suppressed = coastline * (1.0 - suppression_factor * combined_suppression)
            suppressed_coastlines[name] = suppressed

            original_pixels = np.sum(coastline > 0.5)
            suppressed_pixels = np.sum(suppressed > 0.3)
            interior_removed = np.sum((coastline > 0.5) & (ocean_interior_suppression > 0.8))

            print(f"      {name}: {original_pixels:,} -> {suppressed_pixels:,} 像素")
            print(f"               海域内部移除: {interior_removed:,} 像素")

        return suppressed_coastlines, combined_suppression

    def _create_ocean_interior_suppression(self, coastlines, color_regions):
        """创建海域内部完全抑制区域"""
        print("      🌊 创建海域内部抑制区域...")

        blue_mask = color_regions['blue_mask']

        # 合并所有海岸线检测结果，找出可能的海岸线
        combined_coastlines = np.zeros_like(blue_mask, dtype=float)

        # 优先使用重要的海岸线检测结果
        priority_names = ['enhanced_edges', 'ocean_land_interface', 'learned_from_gt']

        for name in priority_names:
            if name in coastlines:
                combined_coastlines += coastlines[name] * 0.5

        # 添加其他检测结果（权重较低）
        for name, coastline in coastlines.items():
            if name not in priority_names:
                combined_coastlines += coastline * 0.2

        # 二值化得到可能的海岸线
        potential_coastline = (combined_coastlines > 0.3).astype(bool)

        if np.sum(potential_coastline) == 0:
            return np.zeros_like(blue_mask, dtype=float)

        # 使用海岸线来定义海域边界
        ocean_interior_mask = self._identify_ocean_interior(potential_coastline, blue_mask)

        # 创建渐变抑制区域
        interior_suppression = self._create_gradient_suppression(ocean_interior_mask, blue_mask)

        interior_pixels = np.sum(ocean_interior_mask)
        print(f"         识别海域内部区域: {interior_pixels:,} 像素")

        return interior_suppression

    def _identify_ocean_interior(self, coastline_mask, blue_mask):
        """识别海域内部区域"""
        from scipy.ndimage import distance_transform_edt, binary_fill_holes

        # 方法1: 距离变换识别海域深处
        distance_to_coastline = distance_transform_edt(~coastline_mask)

        # 在蓝色区域中，距离海岸线较远的区域视为海域内部
        ocean_interior_distance = np.where(blue_mask, distance_to_coastline, 0)

        # 动态阈值：根据图像大小调整
        image_diagonal = np.sqrt(blue_mask.shape[0] ** 2 + blue_mask.shape[1] ** 2)
        min_interior_distance = max(15, image_diagonal * 0.05)

        interior_by_distance = ocean_interior_distance > min_interior_distance

        # 方法2: 形态学操作识别海域中心
        from scipy.ndimage import binary_erosion

        # 对蓝色区域进行大幅腐蚀，剩下的就是海域中心
        erosion_size = max(10, int(min(blue_mask.shape) * 0.08))
        kernel = np.ones((erosion_size, erosion_size), dtype=bool)

        eroded_ocean = binary_erosion(blue_mask, structure=kernel, iterations=2)

        # 合并两种方法
        ocean_interior = interior_by_distance | eroded_ocean

        # 确保只在蓝色区域内
        ocean_interior = ocean_interior & blue_mask

        return ocean_interior

    def _create_gradient_suppression(self, interior_mask, blue_mask):
        """创建渐变抑制区域"""
        from scipy.ndimage import distance_transform_edt

        # 从海域内部边界开始的距离变换
        distance_from_interior = distance_transform_edt(~interior_mask)

        # 在蓝色区域内创建渐变抑制
        gradient_suppression = np.zeros_like(blue_mask, dtype=float)

        # 海域内部：完全抑制
        gradient_suppression[interior_mask] = 1.0

        # 海域内部附近：渐变抑制
        near_interior = blue_mask & (distance_from_interior <= 20) & (~interior_mask)

        for y, x in np.where(near_interior):
            dist = distance_from_interior[y, x]
            # 距离越近，抑制越强
            suppression_strength = max(0.5, 1.0 - dist / 20.0)
            gradient_suppression[y, x] = suppression_strength

        return gradient_suppression
    ...
    def coastline_continuity_enhancement(self, coastlines, color_regions):
        ...
        for name, coastline in coastlines.items():
            if np.sum(coastline) > 0:
                ...
                # 第二步：强制连续性检查和修复
                continuity_fixed = self._enforce_coastline_continuity(connected, color_regions)

                # 第三步：最终平滑
                smoothed = self._smooth_coastline(continuity_fixed)
    ...
    def _enforce_coastline_continuity(self, coastline, color_regions):
        """强制海岸线连续性 - 检测断裂并强制连接"""
        print("      🚨 强制连续性检查...")
        ...
        # 检测所有连通组件
        labeled, num_components = label(binary_coastline)

        if num_components <= 1:
            print("         ✅ 海岸线已连续")
            return coastline

        print(f"         ⚠️ 发现 {num_components} 个断裂片段，正在强制连接...")
        ...
        for i in range(1, num_components + 1):
            ...
            if component_size >= 5:  # 过滤太小的噪声
                endpoints = self._find_coastline_endpoints(component_mask)
        ...
        # 强制连接策略
        connected_coastline = binary_coastline.copy()

        # 按组件大小排序，优先连接大组件
        component_info.sort(key=lambda x: x['size'], reverse=True)

        connection_count = 0
        max_connections = min(10, len(component_info) * 2)  # 限制连接数量防止过度连接
        ...
        for i in range(len(component_info)):
            for j in range(i + 1, len(component_info)):
                ...
                comp1 = component_info[i]
                comp2 = component_info[j]
                ...
                # 强制连接判断
                if best_connection and best_distance <= 50:  # 增加最大连接距离
                    ep1, ep2 = best_connection

                    # 验证连接的合理性
                    if self._validate_forced_connection(ep1, ep2, color_regions, coastline.shape):
                        line_points = self._draw_line(ep1[0], ep1[1], ep2[0], ep2[1])

                        # 绘制连接线
                        for py, px in line_points:
                            if 0 <= py < coastline.shape[0] and 0 <= px < coastline.shape[1]:
                                connected_coastline[py, px] = True

                        connection_count += 1
                        print(
                            f"         ✅ 强制连接: 组件{comp1['id']} -> 组件{comp2['id']} (距离: {best_distance:.1f})")

        print(f"         📊 完成 {connection_count} 个强制连接")

        # 最终连通性验证
        final_labeled, final_components = label(connected_coastline)
        if final_components > 1:
            print(f"         ⚠️ 警告: 仍有 {final_components} 个独立片段，海岸线可能不完整")

        return connected_coastline.astype(float)

    def _validate_forced_connection(self, ep1, ep2, color_regions, shape):
        """验证强制连接的合理性"""
        line_points = self._draw_line(ep1[0], ep1[1], ep2[0], ep2[1])
        ...
        # 连接线应该主要经过海岸线附近区域
        valid_ratio = (ocean_points + land_points) / total_points

        # 放宽连接条件以确保连续性
        return valid_ratio > 0.3  # 降低要求，确保能够连接
    ...
    def intelligent_coastline_fusion(self, coastlines, learned_features):
        ...
        weights = {
            'ocean_boundary': 0.15,
            'land_boundary': 0.12,
            'ocean_land_interface': 0.25,  # 提高海陆交接线权重
            'cnn_edges': 0.10,
            'prewitt_edges': 0.08,
            'texture_edges': 0.08,
            'white_annotations': 0.12,
            'enhanced_edges': 0.35,  # 提高增强边缘权重
        ...
        # 应用连续性惩罚
        continuity_enhanced = self._apply_continuity_penalty(combined)

        threshold = self._calculate_adaptive_threshold(continuity_enhanced)
        final_coastline = (continuity_enhanced > threshold).astype(float)

        final_coastline = self._post_process_coastline(final_coastline)

        # 最终连续性检查
        final_coastline = self._final_continuity_check(final_coastline)

        final_pixels = np.sum(final_coastline > 0.5)
        print(f"✅ 最终海岸线: {final_pixels:,} 像素 (阈值: {threshold:.3f})")

        return final_coastline, continuity_enhanced

    def _apply_continuity_penalty(self, combined_score):
        """应用连续性惩罚机制"""
        print("   ⚖️ 应用连续性惩罚机制...")

        # 检测当前的连通性
        binary_temp = (combined_score > 0.3).astype(bool)
        labeled, num_components = label(binary_temp)

        if num_components <= 1:
            print("      ✅ 无需连续性惩罚")
            return combined_score

        print(f"      ⚠️ 发现 {num_components} 个断裂片段，应用连续性惩罚...")

        # 创建连续性增强版本
        enhanced_score = combined_score.copy()
        ...
        for i in range(1, num_components + 1):
            ...
            if component_size >= 5:
                ...
                # 找到最高得分的点作为连接点
                component_scores = combined_score[component_mask]
                max_score_idx = np.argmax(component_scores)
                best_point = (y_coords[max_score_idx], x_coords[max_score_idx])
        ...
        # 在重要组件之间创建连接路径
        for i in range(len(component_info)):
            for j in range(i + 1, min(i + 3, len(component_info))):  # 限制连接数量
                comp1 = component_info[i]
                comp2 = component_info[j]

                # 计算连接距离
                distance = np.sqrt((comp1['best_point'][0] - comp2['best_point'][0]) ** 2 +
                                   (comp1['best_point'][1] - comp2['best_point'][1]) ** 2)

                if distance <= 60:  # 合理的连接距离
                    # 在两点之间创建连接路径
                    connection_strength = max(0.3, 0.8 - distance / 100.0)
                    self._create_connection_path(enhanced_score,
                                                 comp1['best_point'],
                                                 comp2['best_point'],
                                                 connection_strength)

                    print(
                        f"         连接路径: 组件{comp1['id']} -> 组件{comp2['id']} (强度: {connection_strength:.2f})")

        return enhanced_score

    def _create_connection_path(self, score_map, point1, point2, strength):
        """在两点之间创建连接路径"""
        line_points = self._draw_line(int(point1[0]), int(point1[1]),
                                      int(point2[0]), int(point2[1]))

        for py, px in line_points:
            if 0 <= py < score_map.shape[0] and 0 <= px < score_map.shape[1]:
                # 使用最大值操作，不覆盖已有的高分区域
                score_map[py, px] = max(score_map[py, px], strength)

                # 在路径周围添加轻微的增强
                for dy in [-1, 0, 1]:
                    for dx in [-1, 0, 1]:
                        ny, nx = py + dy, px + dx
                        if (0 <= ny < score_map.shape[0] and
                                0 <= nx < score_map.shape[1]):
                            score_map[ny, nx] = max(score_map[ny, nx], strength * 0.6)

    def _final_continuity_check(self, coastline):
        """最终连续性检查和修复"""
        print("   🔍 最终连续性检查...")
        ...
        print(f"      ⚠️ 警告: 最终海岸线仍有 {num_components} 个独立片段")
        print("         建议检查输入图像质量或调整参数")

        # 尝试最后的连接修复
        final_fixed = binary_coastline.copy()
        ...
        if len(component_sizes) >= 2:
            # 保留最大的两个组件，尝试连接
            main_components = component_sizes[:2]

            for comp_id, size in main_components:
                component_mask = (labeled == comp_id)
                endpoints = self._find_coastline_endpoints(component_mask)

                if len(endpoints) > 0:
                    print(f"         主要组件 {comp_id}: {size:,} 像素, {len(endpoints)} 个端点")

        return final_fixed.astype(float)
