...
class EnhancedCoastlineAgent:
    ...
    def apply_enhanced_inference(self, max_inference_steps=1800):
        ...
        for i, position in enumerate(inference_positions):
            ...
            # 更宽松的更新策略：几乎所有边缘都保留
            # 基础更新值基于边缘强度
            base_update = edge_strength * 0.5 + 0.3  # 大幅提高基础值
            reward_bonus = min(0.4, max(0, reward / 20.0))  # 降低奖励要求
            update_value = min(1.0, base_update + reward_bonus)

            # 极其宽松的更新条件 - 基本上所有候选位置都保留
            if edge_strength > 0.01:  # 几乎所有边缘都保留
        ...
        # 如果像素数量还是太少，进行多轮补充
        if final_pixels < 80000:  # 提高目标到8万像素
            print("   🔧 像素数量不足，进行补充...")
            self._emergency_pixel_supplement(advanced_edges)

            # 第二轮补充
            if np.sum(self.env.current_coastline > 0.3) < 60000:
                print("   🔧 进行第二轮补充...")
