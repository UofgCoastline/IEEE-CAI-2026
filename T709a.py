...
def quick_test_enhanced_v2_with_gt():
    """快速测试增强版 v2.0 + GT对比 (无OpenCV版本)"""
    print("🧪 快速测试增强版 v2.0 + GT对比 (无OpenCV版本)...")
    ...
    if result:
        ...
        # 注意：这里暂时不创建可视化，因为需要更新可视化函数
        # 可以在后续添加无OpenCV版本的可视化
        print("   📊 可视化功能待实现 (无OpenCV版本)")
...
# ==================== 快速测试函数 ====================
    ...
    # ==================== 精确替换你的process_ground_truth方法 ====================

    def process_ground_truth(self, gt_image):
        """处理Ground Truth图像，提取海域和陆域 - 无cv2版本"""
        print("   🔄 处理Ground Truth图像...")

        # 🔧 替换cv2.cvtColor - 手动RGB到灰度转换
        if len(gt_image.shape) == 3:
            # 使用标准的RGB到灰度转换权重
            gt_gray = np.dot(gt_image[..., :3], [0.299, 0.587, 0.114]).astype(np.uint8)
        else:
            gt_gray = gt_image.copy()

        # 🔧 替换cv2.threshold - 手动二值化
        # 原代码：_, gt_binary = cv2.threshold(gt_gray, 128, 255, cv2.THRESH_BINARY)
        gt_binary = np.zeros_like(gt_gray, dtype=np.uint8)
        gt_binary[gt_gray > 128] = 255
        gt_binary[gt_gray <= 128] = 0
    ...
    # ==================== 完整的GroundTruthComparator类替换 ====================

    class GroundTruthComparator:
        """Ground Truth对比评估器 - 完全无cv2版本"""

        def __init__(self, gt_directory="E:/Other_test"):
            self.gt_directory = gt_directory
            print("✅ Ground Truth对比评估器初始化完成 (完全无cv2版本)")
            print(f"   📁 GT目录: {gt_directory}")
        ...
        def compare_with_groundtruth(self, predicted_coastline, city_name, num_regions=30, num_selected=20,
                                     remove_worst=2):
            """与Ground Truth进行对比评估"""
            print(f"\n🔍 开始与Ground Truth对比评估...")
            print(f"   🏙️ 城市: {city_name}")
